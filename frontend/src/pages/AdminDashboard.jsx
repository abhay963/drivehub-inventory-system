...
const AdminDashboard = () => {
  ...
  const stockHealthRadar = useMemo(() => {
    ...
    return [
      { metric: "In stock", score: Math.round((healthyStock.length / total) * 100) },
      { metric: "Low stock", score: Math.round((lowStock.length / total) * 100) },
      { metric: "Out of stock", score: Math.round((outOfStock.length / total) * 100) },
      { metric: "Categories", score: Math.min(100, categoryCount * 14) },
      { metric: "Brand mix", score: Math.min(100, brandData.length * 12) },
    ];
  ...
  const trendData = useMemo(() => {
    ...
    return months.map((month, i) => {
      ...
      return {
        month,
        stock: Math.max(1, Math.round(stock * factor * (0.9 + (i % 3) * 0.05))),
  ...
  return (
    ...
    >
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500">
              Live inventory · admin control center
            </span>
          </div>
          ...
          <p className="mt-1 text-sm text-zinc-500 max-w-xl">
            Fleet overview, stock health, valuation mix, and management tools for your dealership.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-white tabular-nums">
                  {summary.totalStock > 0
                    ? formatINR(Math.round(summary.totalValue / summary.totalStock))
                    : "—"}
                </p>
              </div>
            </div>
        ...
        >
          ...
          <Link
            ...
            className="flex items-center gap-2 text-[13px] text-zinc-300 hover:text-white transition-colors"
          >
            <ShoppingCart className="w-3.5 h-3.5 text-emerald-400" /> Purchase / restock
            <ArrowRight className="w-3 h-3 ml-auto text-zinc-600" />
          </Link>
          <Link
            to="/inventory"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
        ...
        <ChartCard
          title="Stock by Category Pie Chart"
